...
friend class MRFEnergy<TypeGeneral>;
	...
	struct Vector
	{
		...
		REAL ComputeMin(GlobalSize Kglobal, LocalSize K, Label& kMin, REAL *prob);            // return min_k { this[k] }, set kMin
...
inline TypeGeneral::GlobalSize::GlobalSize(int K)
{
}
...
inline TypeGeneral::REAL TypeGeneral::Vector::ComputeMin(GlobalSize Kglobal, LocalSize K, Label& kMin,REAL *prob)
{
	REAL vMin = m_data[0];
	prob[0] = m_data[0];
	kMin = 0;
